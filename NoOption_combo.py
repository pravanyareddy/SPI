...
# GPIO.setup(36, GPIO.OUT)
 ...
 while continue_reading:
    lcd_string("Connecting to server",LCD_LINE_1,2)

    lcd_string('    please wait     ',LCD_LINE_2,2)
...
#            customURL= "http://www.cykul.com/CYKULStations/Python/cycleTracker" 
 #           responseReceived=requests.get(customURL)
  #          parentRootJSONObject = responseReceived.json()
            ...
            #lcd_string(parentRootJSONObject['card_owner_name'],LCD_LINE_4,2)
           # lcd_string('    invalid user    ',LCD_LINE_4,2)
...
def Option_key():
     ...
     col3=[12,16,33]
     for j in range(3):
          GPIO.setup(col3[j],GPIO.OUT)
          GPIO.output(col3[j],1)
     for i in range(4):
          GPIO.setup(row3[i],GPIO.IN,pull_up_down=GPIO.PUD_UP)

     enteredOTP3 = "Enter OTP:"
     def customBreakpoint():
       ...
       while(True):
          ...
          for j in range(3):
               GPIO.output(col3[j],0)
               for i in range(4):
                    if (GPIO.input(row3[i])==0):


                         MATRIX[i][j]
                         NEW_ARRAY3 = map(list,MATRIX)
                         MATRIX[i][j]
                         time.sleep(0.1)
                         ...
                         if(MATRIX[i][j]=='*'):
                                  ...
                                  if(tempLength3>=0):
                                       L= L[:-1]
                                       time.sleep(0.1)
                                       lcd_string(L,LCD_LINE_4,2)
                         else:

                            L=L+str(NEW_ARRAY3[i][j])
                            time.sleep(0.1)
                         ...
                         lcd_string(L,LCD_LINE_4,2)
                         while(GPIO.input(row3[i])==0):
                              pass

               GPIO.output(col3[j],1)
     option=customBreakpoint()
     return option
...
####receiving status of cycle from server###########################################################################################
     ...
     lcd_string("--------------------",LCD_LINE_4,2)
     time.sleep(4)
#     customURL1= "http://www.cykul.com/CYKULStations/Python/cycleTracker.php"
#     responseReceived=requests.get(customURL1)
#     parentRootJSONObject1 = responseReceived.json()
#     print parentRootJSONObject1['response']
#     Card_validation()
    # if(p==0):
     Rasp_Card()
   #    p+=1
 ...
 #      Berry_Card()
    ...
    # if(parentRootJSONObject1['response']=='1'):
   #  if(flag==1):
   #       Rasp_Card()
